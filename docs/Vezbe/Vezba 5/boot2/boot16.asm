...
; sa sistemom datoteka FAT16 (ID sistema datoteka: 4, 6)
...
;------------------------------------------
; Rezervisati memoriju za smestanje FAT16 
;------------------------------------------
        ...
        adc     dx, bx                   ; DX:AX = Logical Block Address (LBA) prvog sektora prve FAT  
...
CitajSledeciKlaster:
        call    CitajKlaster
        cmp     si, 0FFF8h              ; EOC (End of Chain) moze da bude od 0FFF8h do 0FFFFh         
...
;---------------------------------------------------------------------------
; 16-bitne FAT stavke zauzimaju tacno po 2 bajta. 
; Zbog toga je velicina FAT16 tabele 128 KB.  
;----------------------------------------------------------------------------

;---------------------------------
; Citanje FAT16 klastera    
;---------------------------------
; Ulaz:  ES:BX -> bafer
;        SI = broj klastera 
...
; --------------------------------------
; Deo karakteristican samo za FAT16    
; Izlaz: SI = sledeci klaster datoteke
;---------------------------------------

        add     si, si                      ; SI = klaster * 2
        push    ds
        mov     ax, [ss:bp+3*2]             ; DS = memorijski segment gde je smestena FAT16
        jnc     Prvih64
        add     ax, 1000h                   ; Podesiti segment za drugi deo FAT16
Prvih64:
        mov     ds, ax
...
ZavrsenoCitanje:
        ...
        shr     di, 4                   ; Paragrafa po sektoru
        mov     si, es
        add     si, di
        mov     es, si                  ; Podesiti segment za sledeci sektor
